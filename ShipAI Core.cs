     ...
     */
        ...
        void Main()
        {
            ...
            if (!isScreenModulePresent)
            {
                ...
                updateStatusPanels(); //Turn this off if you have the AI Screen Module.
            }
        ...
        List<IMyTextPanel> configPanels = new List<IMyTextPanel>();
        IMyTextPanel debugPanel;
        IMyTextPanel storagePanel;

        IMyProjector healingProjector;
        Dictionary<string, int> missingBlocks = new Dictionary<string, int>();


        List<IMySoundBlock> speakersBlue = new List<IMySoundBlock>();
        ...
        string getDamageReport(bool listOfflineBlocks)
        {
        ...
        void soundAlerts()
        {
            if (codeBlue)
            {
                for (int i = 0; i < speakersBlue.Count; i++)
                {
                    ...
                    speakersBlue[i].ApplyAction("PlaySound");
                    if (!speakersBlue[i].IsSoundSelected)
                    {
                        debugOutput("No sound selected on " + speakersBlue[i].DefinitionDisplayNameText);
                    }
                }
            }
            else if (codeOrange)
            {
                for (int i = 0; i < speakersOrange.Count; i++)
                {
                    ...
                    speakersOrange[i].ApplyAction("PlaySound");
                    if (!speakersOrange[i].IsSoundSelected)
                    {
                        debugOutput("No sound selected on " + speakersOrange[i].DefinitionDisplayNameText);
                    }
                }
            }
            else if (codeRed)
            {
                for (int i = 0; i < speakersRed.Count; i++)
                {
                    ...
                    speakersRed[i].ApplyAction("PlaySound");
                    if (!speakersRed[i].IsSoundSelected)
                    {
                        debugOutput("No sound selected on " + speakersRed[i].DefinitionDisplayNameText);
                    }
                }
            }
        }
        ...
        {
            var power = getWattHourInBatteries();
            bool toggleRefineries = true;
            for (int i = 0; i < refineries.Count; i++)
            {
                if (minimumKWh <= 0)
                {
                    if (power <= (getMaxWattHourInBatteries() / 4)) { debugOutput("Turning off refinery " + i + " to preserve power."); refineries[i].ApplyAction("OnOff_Off"); }
                    else if (power >= ((getMaxWattHourInBatteries() / 4) + toggleOffset)) { debugOutput("Turning on refinery " + i + " as there's enough power."); refineries[i].ApplyAction("OnOff_On"); }
                }
                else
                {
        ...
        void manageAssemblers(int toggleOffset = 350, int minimumKWh = 0)
        {
            ...
            for (int i = 0; i < assemblers.Count; i++)
            {
                if (minimumKWh <= 0)
                {
                    if (power <= (getMaxWattHourInBatteries() / 4)) { debugOutput("Turning off assembler " + i + " to preserve power."); assemblers[i].ApplyAction("OnOff_Off"); }
                    else if (power >= ((getMaxWattHourInBatteries() / 4) + toggleOffset)) { debugOutput("Turning on assembler " + i + "as there's enough power."); assemblers[i].ApplyAction("OnOff_On"); }
                }
                else
                {
        ...
        /// <summary>
        /// Reactor management is really a matter of personal taste, 
        /// but I personally want solar and battery power to be used before reactors become active.
        /// I also don't want reactors to charge batteries. which is the only thing you can toggle in this scheme
        /// If you don't like the reactor scheme, build your own.
        /// </summary>
        /// <param name="chargeBatteries">Whether or not to charge batteries using reactor power</param>
        void manageReactors(bool chargeBatteries = false)
        {
            //updateMisc(); //Might not be needed, if updateStatusPanel() is called first.
            if (batteries.Count < 1 && solarpanels.Count < 1)
            { return; } //It would be unwise to turn off our only known energy source.


            if (maxPower < powerUsage || solarpanels.Count < 1) //If Solar power doesn't cut it
            {
                if (wattHourInBatteries < (maxWattHourInBatteries / 4)) //If battery is low
                {
                    ...
                    {
                        debugOutput("Turning off batteries (!chargebatteries)");
                        for (int i = 0; i < batteries.Count; i++)
                        {
                            batteries[i].ApplyAction("OnOff_Off"); //Turn off all batteries
                            debugOutput("Turning off batteries");
                        }
                    }
                }
            }
            else if (solarpanels.Count > 0)//If solar power is enough to charge the batteries and there are solar panels
            {
                debugOutput("Solar output high enough to charge batteries.");
                debugOutput("Turning off all reactors, and setting all batteries to charge");
                for (int i = 0; i < batteries.Count; i++)
                {
                    batteries[i].ApplyAction("OnOff_On"); //Turn on all batteries
                }
                for (int i = 0; i < reactors.Count; i++)
                {
                    reactors[i].ApplyAction("OnOff_Off"); //Turn off all reactors
                }
            }
        ...
        /// <summary>
        /// Original battery management code written and tested by Stiggan and Malakeh in January 2015.
        /// </summary>
        void manageBatteries()
        {
            // Raise the usage by this much in kW to make batteries kick in earlier.  
            int offset = 0;

            // Modify total on the reactors maximum to make batteries kick in earlier.  
            float margin = 1.00f;


            maxPower = (int)(margin * ((float)maxPower));
            powerUsage += offset * 1000;

            if (maxPower > powerUsage)
            {
                toggleRandomBattery(); // Toggle on a battery to see if power is needed.
                chargeAllBatteries(); // Sets batteries to charge.
            }
            else
            {
                chargeAllBatteries(); // Sets batteries to discharge.
            }
        }
